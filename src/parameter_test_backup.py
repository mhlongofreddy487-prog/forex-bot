...
def signal(prices, fast, slow, trend, rsi_period, strength):

    minimum_history = max(slow, trend, rsi_period + 1)
    ...
    recent_avg = moving_average(prices, 5)
    ...
    if fast_ma > slow_ma and slow_ma > trend_ma and 50 < rsi < 70:
        return "BUY"

    if fast_ma < slow_ma and slow_ma < trend_ma and 30 < rsi < 50:
        return "SELL"

    return "HOLD"


def test_strategy(candles, fast, slow, trend, rsi_period, strength):
...
if __name__ == "__main__":
    ...
    rsi_values = [10, 14, 20]
    strength_values = [0.0003, 0.0005, 0.0007]
